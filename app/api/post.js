    ...
    app.post('/api/post/detail', function (req, res) {
        ...
        pool.getConnection(function (err, connection) {
            if (err) {
                ...
                if (req.body.subCategory == "thong-bao-hot-news") {
                    connection.query('SELECT post.* FROM post WHERE post.sub_category_id = 23 AND post.status_id = 3', function (err, rows) {
                        res.json(rows.length > 0 ? rows: {});
                        connection.destroy();
                        pool.end();
                    });
                } else if (req.body.subCategory == "cau-lac-bo-UTEC") {
                    connection.query('SELECT post.* FROM post WHERE post.sub_category_id = 24 AND post.status_id = 3', function (err, rows) {
                        res.json(rows.length > 0 ? rows: {});
                        connection.destroy();
                        pool.end();
                    });
                } else if (req.body.subCategory == "cau-lac-bo-BKDEC") {
                    connection.query('SELECT post.* FROM post WHERE post.sub_category_id = 25 AND post.status_id = 3', function (err, rows) {
                        res.json(rows.length > 0 ? rows: {});
                        connection.destroy();
                        pool.end();
                    });
                } else if (req.body.subCategory == "tai-lieu-hoc-tap") {
                    connection.query('SELECT post.* FROM post WHERE post.sub_category_id = 26 AND post.status_id = 3', function (err, rows) {
                        res.json(rows.length > 0 ? rows: {});
                        connection.destroy();
                        pool.end();
                    });
                } else if (req.body.subCategory == "blog" && req.body.slug == "") {
